...
(defn run
  "Random-restart hill climbing using first better neighbor at each step.
   If spends steps-before-restart steps without finding better neighbor,
   randomly restarts."
  [{:keys [genome-factory individual-factory stop-fn individual-cmp mutate steps-before-restart max-steps]}]
  (let [rand-individual #(i/make-individual individual-factory (genome-factory))]
    (loop [step 0
      ...
      (let [result (stop-fn {:step step :individual individual :max-steps max-steps})]
        (cond (some? result)
              {:step       step
               :result     result
